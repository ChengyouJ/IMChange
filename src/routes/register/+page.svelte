...
<div class="min-h-[calc(100vh-10rem)] flex items-center justify-center px-4 py-8">
    <Card class="w-full max-w-lg shadow-lg">
        <CardHeader class="space-y-1">
            <CardTitle class="text-3xl font-bold text-center">Join the Network</CardTitle>
        ...
        <CardContent>
            <form method="POST" class="space-y-4">
                <div class="space-y-2">
                    ...
                    <Input id="name" name="name" type="text" placeholder="Food Bank North" required />
                </div>

                <div class="space-y-2">
                    <label for="email" class="text-sm font-medium">Email</label>
                    <Input id="email" name="email" type="email" placeholder="contact@example.org" required />
                </div>
                ...
                <div class="space-y-2">
                    <label class="text-sm font-medium">Address Lookup</label>
                    <p class="text-xs text-muted-foreground">Enter your UK postcode to find your address</p>
                    ...
                    <div class="flex gap-2">
                        <input
                            ...
                            class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm uppercase"
                            on:keydown={(e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                        ...
                        <button
                            ...
                            disabled={!canSearch}
                            class="h-10 px-4 py-2 whitespace-nowrap bg-primary text-primary-foreground rounded-md hover:bg-primary/90 disabled:opacity-50 disabled:pointer-events-none font-medium"
                        >
                    ...
                    {#if selectedAddress}
                        <div class="p-3 bg-primary/10 border border-primary/20 rounded-md">
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-primary">Selected Address:</p>
                                    <p class="text-sm mt-1">{selectedAddress.address_line_1}</p>
                                    <p class="text-xs text-muted-foreground mt-1">{selectedAddress.display_name}</p>
                                </div>
                                <button
                                    type="button"
                ...
                <div class="space-y-2">
                    <label for="contact_info" class="text-sm font-medium">Contact Info (Phone/Link)</label>
                    <Input id="contact_info" name="contact_info" type="text" placeholder="+44 7700 900000" />
                </div>

                {#if form?.missing}
                    <div class="p-3 bg-red-500/10 border border-red-500/20 text-red-700 dark:text-red-400 rounded-md text-sm">
                ...
                <button
                    ...
                    class="w-full h-10 px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 disabled:opacity-50 disabled:pointer-events-none font-medium"
                >
                    Create Account
                </button>
            </form>
            ...
            <div class="mt-6 text-center text-sm">
                ...
                <a href="/login" class="font-medium text-primary hover:underline ml-1">Log in</a>
            </div>
        </CardContent>
    </Card>
</div>
